...
| 4 | [Two ways a long-context agent session burns money, neither of them thinking](entries/context-burn-two-modes/) | Nothing breaks. Turns complete, tools run, tests pass — and one 131-second turn that produced 3,080 tokens costs $20.70, while two unrelated sessions spike 81 seconds apart | ![trace replay](https://img.shields.io/badge/evidence-trace%20replay-blue) ![mechanism simulation](https://img.shields.io/badge/evidence-mechanism%20simulation-yellow) | 91% of one session's cost was moving context around rather than generating anything; generation was 9.2%. The natural hypothesis — "it's thinking too hard" — is plausible and worth less than a tenth of the bill | Watch the quantity that scales: context tokens as a fraction of the window, per live session. Retire a session at a watermark instead of handing it the next task — the CLI already reports `contextWindow` on every turn, and nothing reads it |
