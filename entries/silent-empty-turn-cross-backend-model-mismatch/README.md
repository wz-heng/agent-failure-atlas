...
- **The silent shape is not reproducible from the incident's configuration on
  either build today — and where the difference lives is undetermined.** A
  server-side change is consistent with the captures, and it is my prior; I
  cannot offer it as a finding, because the observation that would support it —
  the incident's own binary being loud today — is confounded by the same
  differences that make everything else here undetermined. These captures
...
top of that file, applied to a throwaway copy of this directory. It refuses to
run if the baseline is not green or if a mutation no longer matches exactly once.

<!-- generated by mutations.py -->

| mutation | tests red | `repro.py` |
|---|---|---|
| delete the standalone `error` branch | 3 | red |
| delete the `turn.failed` branch | 3 | red |
| disable guard 1 (the empty-turn check) | 4 | red |
...
**Why this is generated.** Two earlier revisions of this section quoted
hand-transcribed mutation numbers, and both were wrong — the second one in the
paragraph that was apologising for the first.

A reviewer and I then reported different counts for what we both called
"delete the `error` branch". Neither of us was miscounting: we had made
*different edits*. Deleting the whole branch lets the record fall through to the
top-level handler, which appends to `debug_logged`, so
`test_the_shipped_parser_drops_the_cli_s_own_warning` goes red as well;
replacing the branch's body with `return []` consumes the record silently and it
does not. One extra test sees the difference. Neither of us had written down
what we edited.

That is the whole argument for this file. Numbers a reader cannot regenerate do
not belong in an entry whose thesis is *assert on what it produced*, and a
mutation is not specified until the exact edit is declared — which is what
`MUTATIONS` is. `python3 mutations.py --check` asserts the table above still
matches what the generator produces, so this surface cannot drift a third time
in silence.

*(An earlier revision of this paragraph blamed the disagreement on the two of us
counting different things — failure lines versus failed tests. That was a cause
asserted rather than measured, in the paragraph explaining why asserted claims
are untrustworthy. The reviewer measured it and it was wrong. It is recorded
