...
paragraph that was apologising for the first. A reviewer and I then measured
different counts for the same mutation, because we were counting different
things (failure *lines* versus failed *tests*, which differ when an assertion
message wraps). Numbers a reader cannot regenerate do not belong in an entry
whose thesis is *assert on what it produced*, so these are regenerated by one
command and any disagreement is settled by running it.
