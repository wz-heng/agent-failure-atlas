...
> the arm the ledger cannot contain: *the same synthetic task sequence, on the
> same output-token budget, under different session hygiene*. Its numbers are invented and are never mixed with the
...
each. (Of those rows, **15** carry usage; the other five are aborted turns with
every counter at zero, and the entry counts them wherever it matters.)
...
**Four turns do not reconcile**, and they are not spread evenly — which
changes what can be said about each session total:

| session | priced turns | reconciling | so its total is |
|---|---:|---:|---|
| A | 15 | **15** | both a provider-reported sum **and** a complete decomposition |
| B | 60 | **56** | a provider-reported sum **only** — 4 turns cannot be decomposed |
| C | 24 | **24** | both |

So session A's **$53.85** is a full decomposition and the mode-A table above is
complete. Session B's **$235.7484** is a sum of the reported `cost` field with
four undecomposable turns inside it, and is quoted on that basis alone. An
earlier revision of this paragraph lumped the two together and understated what
session A supports; a reviewer recomputed it.

The strangest non-reconciling row is an errored turn billed **$9.8231** with
every token counter at zero — a charge with no recorded consumption. I do not
know what it is. `claims.py` pins the count at **four** so this paragraph cannot
quietly become "everything reconciles".
...
**codex** backend records `cache_creation_tokens` as 0 on *every* turn — and
that is a fact about the adapter, not an inference from six rows. Owlery's
codex usage normalizer (`server/harness/codex.py`, `_normalize_usage`) builds
its `TokenUsage` from the four fields Codex actually emits — `input_tokens`,
`cached_input_tokens`, `output_tokens`, `reasoning_output_tokens` — and
**never assigns `cache_creation_tokens` at all**, so it takes its default of
zero. The stored zero means "not reported", not "nothing was written".
...
A reviewer demonstrated both holes, then a third: a bare `literal in readme`
test is not a location check either, because most of these figures appear
several times across the English text, the tables and the Chinese summary — so
changing *one* of three occurrences left it green. `claims.py` therefore pins,
per claim, an exact **match count per file**, across this README and both root
READMEs. Change one occurrence of nine and the count drops to eight and it goes
red. It also carries a SHA-256 per trace, so an edit to a row no claim happens
to name is caught too.

Its own limitation is documented in the file rather than left implied: counts
...
**Detection in production**: one gauge and one counter, and they are not
equally available today.

- **Gauge — context tokens as a fraction of the window, per live session.** This
  is the watermark, and it **needs the new field described above**. It cannot be
  derived from what is stored.
- **Counter — turns whose cache-write share is high**, where the share is
  `cache_creation / (cache_creation + cache_read)`: the fraction *of the cache
  tokens that turn touched* that had to be written rather than read. That **is**
  computable from what is already stored, and it is exactly the quantity oracle 4
  uses. Note what it is not: it is a ratio over one turn's cache traffic, **not**
  a fraction of the session's context, which — per the paragraph above — is a
  number this ledger does not contain.

The gauge tells you which sessions are expensive to keep warm; the counter tells
you when you were billed to re-warm one. Neither needs to know what the agent is
doing.
...
**证据等级(两级,严格不混用)**:计费重建是 `trace replay`——`repro.py` 回放四个真实会话共 119 条脱敏逐轮用量记录。**关键一步是计价模型经过验证而非假定**:用公开 list price 重算每轮成本,与 CLI 自报的成本比对,在 **2.00×**(**1 小时** TTL 写入价)下 **99 个计费轮中 95 轮**误差 <0.1%,而按 5 分钟 TTL 的 1.25× **一轮都对不上**。由此**能**推出两件事:(a) 对那 95 轮,逐行分解是把供应商自己的算术读回来,不是估算;(b) CLI 的成本计算器用的是 **1 小时**缓存 SKU。但**不能**推出的是:**任何一条具体缓存条目「存活了一小时后过期」**——这个对账识别的是**被计费的费率**(价目表的事实),不是某条缓存条目的生存期;模式 B 的机制只按这个强度陈述,不再更强。**有四轮无法对账,而且它们并非均匀分布**,这改变了每个会话总额各自能说什么:A 会话 15 个计费轮**全部**对账,所以 **$53.85** 既是供应商报告的求和**也是**一份完整分解,上面模式 A 的表是完整的;C 会话 24 轮亦然;只有 B 会话是 60 个计费轮中 56 轮对账、**4 轮无法分解**,因此 **$235.7484** 仅作为「原样求和」引用。早先版本把两者混为一谈、低估了 A 会话所能支撑的结论,是复核重算后指出的。最离奇的一条是**所有 token 计数为零却被计费 $9.8231** 的错误轮次,我不知道那是什么。反事实部分是 `mechanism simulation`:账本记录了马拉松会话被计费多少,却无法记录**同样的工作在不同会话卫生下会花多少**,因为那次运行从未发生;`simulate.py` 用假时钟与假 TTL 缓存跑两条臂(16 个**合成**任务、两臂**输出 token 预算相同**;不生成任何文本,因此这是 token 记账层面的对照,并未论证两臂工作在语义上等价),报出 **1.84×**——**这个数字是我所选参数的性质**,换参数就变,它只用来展示机制的**形状**,并被隔离在独立文件、独立横幅、独立评级之下;**本条目没有任何一张表把模拟数字与实测数字混在一起。**
...
**通用教训**:**当一个折扣大到一定程度,它就不再是节省,而变成盲区。** 缓存把重发上下文的价格砍掉 90%,而这恰恰是「220 : 1 的读放大」能连跑四小时而毫无异样的原因——**单位数字变好了,总额变坏了,而屏幕上只有单位数字。要给会规模化的那个量装仪表,而不是给变便宜的那个量装。** 生产检测:一个 gauge 加一个 counter,而两者**今天的可得性并不相同**。**gauge——每个活跃会话的上下文 token 占窗口比例**,这就是水位线,它**需要上面说的那个新字段**,无法从现有存储推出。**counter——写入占比高的轮次数**,其中占比定义为 `cache_creation / (cache_creation + cache_read)`,即**该轮所触及的缓存 token 中,不得不写入而非读取的那一部分**;这个**用现有存储就能算**,也正是 oracle 4 使用的量。但要说清它**不是**什么:它是对**单轮缓存流量**的比值,**不是**「占会话上下文的比例」——后者按上一段所述,是这个账本里并不存在的数。前者告诉你哪些会话「捂热很贵」,后者告诉你何时被计费去重新捂热一个会话。
