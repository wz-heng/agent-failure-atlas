...
The confirmed cause was contention between two leaked processes on the same
machine:
...
| holds ports, locks, memory? | **yes** | **no** — the kernel released everything at exit |
...
your port is busy.** `repro.py` oracle 3 puts a real orphan and a real zombie
side by side and prints both states, because the words get swapped constantly
and the swap costs debugging hours.

## 3. Evidence

This entry has **two claims at two different evidence levels**, and mixing them
up would be exactly the overreach this atlas is supposed to avoid.
...
seconds, on any POSIX machine. Nothing about it is simulated or replayed. If it
stops reproducing, it fails loudly.
...
**Known limits.** POSIX only — the case needs process groups and `ps`, and
refuses to run elsewhere rather than pretending. It demonstrates one resource
(a TCP port) standing in for a class (ports, file locks, database handles); a
port was chosen because it is exclusive, observable, and behaves the same on
macOS and Linux. `ps` state letters are read as prefixes (`Z…`, `S…`) because
the suffixes differ between platforms.
...
Offline, stdlib only, ~4 seconds, imports nothing from Owlery. It re-executes
itself in three roles (`leader`, `holder`, `exiter`) so the whole case stays in
one readable file. It cleans up everything it spawns — verified by checking for
strays after each run.
...
1. **Leader-only kill leaks, and the leak blocks.** Spawn leader → grandchild;
   ...
   grandchild is still alive, reparented (`ppid 1`), still holding the port —
   and that the next run, given a bounded 2-second window, **fails to acquire
   it**. The timeout is the verdict; you never wait on a real hang.
2. **Process-group kill releases.** Same setup, but `killpg` the group and reap.
   Assert the port is free and the grandchild is gone.
3. **An orphan is not a zombie.** A live orphan and a genuine unreaped zombie,
   side by side, with their real `ps` states and parents asserted — the orphan
   `S`/`R` holding the port, the zombie `Z` holding nothing.
...
7 tests, ~1.5s: the bug pinned, the fix, run-teardown-run, teardown idempotency
on an already-dead job, no zombie left by the supervisor itself, the
zombie-holds-nothing property, and a guard that the blocking demo stays bounded.
...
**What Owlery actually does.** Reported as found, not idealised. At its CLI
spawn seam Owlery gets this right, and says why in the code:
...
`prepare_spawn` sets `start_new_session=True`; `_terminate_process_group`
resolves the pgid and `killpg`s it, guarded for `ProcessLookupError` /
`PermissionError` with a fallback to the direct child; teardown escalates
stdin-close → `SIGTERM` group → `SIGKILL` group, awaiting the process between
steps. The design is written down in `docs/plans/turn-safety.md` §2.
...
**防御**:修复有两半,少任何一半仍然泄漏——(1) 用 `start_new_session=True` 把作业生成到自己的进程组;(2) teardown 时对**进程组**发信号(`SIGTERM` → `SIGKILL`)**并 reap**;只发信号不 reap,等于把泄漏的孤儿换成堆积的僵尸。**Owlery 的真实情况**如实报告:它在 CLI spawn 接缝处做对了(`server/harness/run.py` 的 `prepare_spawn` + `_terminate_process_group`,设计记在 `docs/plans/turn-safety.md` §2);但 07-17 泄漏的两个进程都**不是** CLI turn 进程,而是诞生于该接缝之外的 `pytest`——**在你想到的那个子系统里把进程组卫生做对,并不能保护整台机器**。通用教训:**只处理你手上恰好握着的那个句柄的清理路径,不算清理路径**;操作系统里的所有权是一棵树,释放一个节点不会释放它的子树。
